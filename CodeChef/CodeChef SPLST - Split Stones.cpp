// CodeChef SPLST - Split Stones.cpp
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, m, k, cnt = 0, len, a, b, c, x, y;
    ...
    while(T--)
    {
        ...
        bool ok = false;
        if(((x - b) >= 0 && (y - c) >= 0 && (x - b) + (y - c) == a) || ((x - c) >= 0 && (y - b) >= 0 && (x - c) + (y - b) == a))
            ok = true;
        if(((x - a) >= 0 && (y - c) >= 0 && (x - a) + (y - c) == b) || ((x - c) >= 0 && (y - a) >= 0 && (x - c) + (y - a) == b))
            ok = true;
        if(((x - a) >= 0 && (y - b) >= 0 && (x - a) + (y - b) == c) || ((x - b) >= 0 && (y - a) >= 0 && (x - b) + (y - a) == c))
            ok = true;
