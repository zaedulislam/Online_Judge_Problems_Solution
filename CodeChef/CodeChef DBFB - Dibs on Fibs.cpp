// CodeChef DBFB - Dibs on Fibs.cpp
...
LL A[SIZE], B[SIZE], fib[SIZE], newar[SIZE];
...
int main()
{
	...
	///READ();
	///WRITE();
	LL T, I, J, K, N, n, m, k, cnt = 0, len, M, b, aSum, bSum, result;
	...
	while(T--)
	{
		...
		if(N <= 100 && M <= 100)
		{
			for(I = 1; I <= M; I++)
			{
				for(J = 1; J <= M; J++)
				{
					...
					for(K = 3; K <= N; K++)
						newar[K] = ((newar[K - 1] % MOD) + (newar[K - 2] % MOD)) % MOD;

					result = ((result % MOD) + (newar[N] % MOD)) % MOD;
		...
		for(I = 1; I <= M; I++)
		{
			aSum = ((aSum % MOD) + (((((fib[N] % MOD) * (A[I] % MOD)) % MOD) % MOD) * (M % MOD)) % MOD) % MOD;
			bSum = ((bSum % MOD) + (((fib[N + 1] % MOD) * (b % MOD)) % MOD) % MOD) % MOD;
		}


		result = ((aSum % MOD) + (bSum % MOD)) % MOD;
