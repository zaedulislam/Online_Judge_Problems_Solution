// CodeChef MTYFRI - Mighty Friend.cpp
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, m, k, cnt = 0, len, mPos, tPos, maxIndex, minIndex;
	...
	while(T--)
	{
		...
		for(I = 1; I <= J; I++)
		{
			...
			if(mPos < 0 || tPos == tomu.size())
				break;
 
			maxIndex = motu[mPos].second;
			minIndex = tomu[tPos].second;
