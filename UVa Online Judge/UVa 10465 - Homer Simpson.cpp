...
int dp[SIZE], mn = INF, T = 0;

int GetMaxBurgers(int m, int n, int time)
{
	if(time == 0)
		return 0;
	...
	T = time;
	int ret1 = 0, ret2 = 0;
	ret1 = 1 + GetMaxBurgers(m, n, time - m);
	ret2 = 1 + GetMaxBurgers(m, n, time - n);

	dp[time] = MAX(ret1, ret2);
	return dp[time];
}
...
{
	///BOOST
	int tcases, I, J, K, N, n, m, cnt = 0, len, t, w;
	///READ();
	///WRITE();
	while(SF("%d%d%d", &m, &n, &t) != EOF)
	{
		if(m > t && n > t)
		{
			PF("0 %d\n", t);
