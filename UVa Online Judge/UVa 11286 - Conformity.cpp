...
int mat[SIZE][10];
map<pair<pair<int, int>, pair<pair<int, int>, int> >, int> MAP;
...
int main()
{
    ...
    while(1)
    {
        ...
        for(I = 0; I < n; I++)
        {
            ...
            MAP[MP(MP(mat[I][0], mat[I][1]), MP(MP(mat[I][2], mat[I][3]), mat[I][4]))]++;
        }


        int mx = 0;
        map<pair<pair<int, int>, pair<pair<int, int>, int> >, int> :: iterator it = MAP.begin();
        while(it != MAP.end())
        {
            if(it -> second > mx)
                mx = it -> second;
