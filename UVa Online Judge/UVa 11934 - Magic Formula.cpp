...
int main()
{
	...
	int tcases, caseno = 1, n, I, J, K, N, cnt = 0, len, a, b, c, d, L;

	while(sf("%d%d%d%d%d",&a, &b, &c, &d, &L), a || b || c || d || L){
		cnt = 0;
		I = 0;
		while(I <= L){
			LL funcValue = a * I * I + b  * I + c;
			if(funcValue % d == 0) cnt++;
			I++;
		}
		pf("%d\n",cnt);
	}
