// UVa 11536 - Smallest Sub-Array.cpp
...
const int SIZE = 1000001 + 10;
int ar[SIZE], freq[1001 + 10];
deque<int> DQ;




int SmallestSubarray(int N, int K)
{
    int I, cnt = 0, minLength = INTMAX;
    for(I = 0; I < N; I++)
    {
        if(ar[I] <= K)
        {
        ...
        if(cnt == K)
        {
            ...
            {
                minLength = min(minLength, DQ.back() - DQ.front() + 1);

                if(freq[ar[DQ.front()]] - 1 == 0)
                    cnt--;
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, M, m, k, cnt = 0, len;
    ...
    while(T--)
    {
        ...
        int windowLength = SmallestSubarray(N, K);

        PC;CN;
        if(windowLength == INTMAX)
            cout << "sequence nai\n";
        else
