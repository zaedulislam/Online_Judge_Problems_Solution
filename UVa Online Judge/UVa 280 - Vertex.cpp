...
int main()
{
	...
	int tcases, caseno = 1, n, I, J, K, N, cnt = 0, len, node, n1, n2, src;

	while (sf("%d", &node), (node || 0)){
		FOR(I, 0, 150) G[I].clear();
		...
		while (tcases--){
			...
			FOR(I, 1, node + 1){
				if (!visited[I]) inaccessible++;
			}

			pf("%d", inaccessible);
			FOR(I, 1, node + 1){
				if (visited[I] == 0) pf(" %d", I);
			}
			pf("\n");
		}
	}
