...
string S, T;
char s[SIZE];
int ar[SIZE], qry[SIZE];
int M[1000][1000];

int main()
{
	//std::ios_base::sync_with_stdio(0);
	int tcases, caseno = 1, n, I, J, K, N, cnt = 0, len, Q, flag, first, last, mid, value, index;

	while(sf("%d%d", &N, &Q), N || Q){
		sf("\n");
        FOR(I,0,N) sf("%d", &ar[I]);
        ...
        FOR(I,0,Q){
			...
			while(first <= last){
				...
				else if(ar[mid] > value) last = mid - 1;
				else first = mid + 1;
			}
			if(flag) pf("%d found at %d\n",value, index + 1);
			else pf("%d not found\n", value);
