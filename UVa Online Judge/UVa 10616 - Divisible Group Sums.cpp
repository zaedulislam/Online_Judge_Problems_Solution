// UVa 10616 - Divisible Group Sums.cpp
...
int ar[SIZE], D, M;
LL dp[SIZE][10 + 5][20 + 5];

int F(int pos, int N, int cnt, LL taken)
{
	if(cnt == M)
	{
		if(taken == 0)
	...
	if(dp[pos][cnt][taken] != -1)
		return dp[pos][cnt][taken];
	...
	LL ret1 = 0, ret2 = 0;
	ret1 = F(pos + 1, N, cnt + 1, sum);
	ret2 = F(pos + 1, N, cnt, taken);

	dp[pos][cnt][taken] = ret1 + ret2;
	return dp[pos][cnt][taken];
}
