...
const int SIZE = 1e6;
vector<string> dictionary, word, anagram;
string S;
...
int main()
{
    ...
    while(T--)
    {
        ...
        for(I = 0; I < SZ; I++)
        {
            ...
            {
                string a = word[I], b = dictionary[J];

                sort(a.begin(), a.end());
                sort(b.begin(), b.end());

                if(a == b)
                    anagram.PB(dictionary[J]);
            }


            cout << "Anagrams for: " << word[I] << NL;
            if(anagram.size() == 0)
                cout << "No anagrams for: " << word[I] << NL;
            else
            {
                for(K = 0; K < anagram.size(); K++)
                    cout << "  " << K + 1 << ") " << anagram[K] << NL;
            }
