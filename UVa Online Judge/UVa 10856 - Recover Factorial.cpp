// UVa 10856 - Recover Factorial.cpp
#include <iostream>
#include <cstdio>
...
int prime[SIZE], status[SIZE], factor[SIZE];
...
int main()
{
	...
	int tcases, caseno = 1, n, I, J, K, N, cnt = 0, len, first, last, mid, fact;
	sieve();
	factorizeFactor();
	...
	while (sf("%d", &N), (N >= 0)){
		...
		while (first <= last && !found){
			...
			else if (factor[mid] > N) last = mid - 1;
			else first = mid + 1;
		}
		if (found) pf("Case %d: %d!\n", caseno++, fact);
