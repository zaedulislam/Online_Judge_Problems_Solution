// SPOJ BROKEN - Broken Keyboard.cpp
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, m, k, cnt = 0, len, decimal, largestSubstrLen, idx;
    ...
    while(1)
    {
        ...
        for(I = 0; I < len; I++)
        {
            ...
            if(cnt > m)
                largestSubstrLen = max(largestSubstrLen, DQ.back() - DQ.front());

            if(I == len - 1 && cnt <= m)
                largestSubstrLen = max(largestSubstrLen, DQ.back() - DQ.front() + 1);
                
            if(cnt > m)
            {
                while(!DQ.empty() && cnt > m)
                {
                    idx = DQ.front();
                    decimal = (int)S[idx];
                    DQ.pop_front();
