...
#define SF scanf
#define PF printf
#define PC() printf("Scenario #%d: ", caseno++)//NOTES:printf
...
int ar[SIZE], dp[SIZE][SIZE], CAP, N;

int Function(int pos, int taken)
{
    if(pos >= N)
        return 0;
    ...
    int ret1 = 0, ret2 = 0;
    if(ar[pos] + taken <= CAP)
        ret1 = ar[pos] + Function(pos + 2, ar[pos] + taken);

    ret2 = Function(pos + 1, taken);
