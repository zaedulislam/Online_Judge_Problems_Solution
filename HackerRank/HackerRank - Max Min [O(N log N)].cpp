// HackerRank - Max Min [O(N log N)].cpp
#include <iostream>
#include <cstdio>
...
int main()
{
	...
	int tcases, I, J, K, N, n, m, cnt = 0, len, unfairness = INF;
    //READ();
    //WRITE();
	cin >> N >> K;
	...
	sort(ar, ar + N);   // O(N log N)

	for(I = 0; I <= N - K; I++)     // O(N - K)
	{
		if(ar[I + K - 1] - ar[I] < unfairness)
			unfairness = ar[I + K - 1] - ar[I];
	}
	cout << unfairness << NL;
