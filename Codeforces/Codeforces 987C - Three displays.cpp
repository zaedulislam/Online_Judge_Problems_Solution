// Codeforces 987C - Three displays.cpp
...
int main()
{
    ...
    int minI, minJ, minK, minCost = INTMAX;
    bool ok = false;
    for(J = 1; J < n - 1; J++)
    {
        minI = INTMAX;
        ...
        {
            if(ar[J] < ar[K])
                minK = min(minK, cost[K]);
        }

        minJ = cost[J];
        
        if(minI != INTMAX && minJ != INTMAX && minK != INTMAX)
        {
            minCost = min(minCost, minI + minJ + minK);
            ok = true;
        }
