...
{
    long double totalW = (long double)m + w;
    b[n + 1] = b[1];

    int I; long double used;
    for(I = 2; I <= n + 1; I++)
    {
        used = (totalW / a[I - 1]);
        totalW -= used;
...
{
    long double first = 0.0, last = 1000000000000000000.0, mid, ans = -1, I = 1000;
    while(I--)
    {
        mid = (first + last) / 2;
        if(Check(mid))
...
int main()
{
    ...
    double ret = BinarySearch();
    if(ret == -1)
        cout << -1 << NL;
    else
        cout << setprecision(9) << fixed << ret << NL;
