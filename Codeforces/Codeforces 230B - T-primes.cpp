...
int main()
{
	...
	LL tcases, caseno = 1, n, I, J, K, N, cnt, len, temp,var;

	LL sqrtN = int(sqrt((double)sz));
	...
	Tprime[2] = 4;
	for (I = 3; I <= sz; I += 2){
		if (!status[I]) Tprime[I] = POW(I,2);
	}

	sf("%I64d", &n);
	FOR(I, 0, n){
		sf("%I64d", &var);
		if (Tprime[int(sqrt((double)var))] == var)
			pf("YES\n");
		else pf("NO\n");
	}
