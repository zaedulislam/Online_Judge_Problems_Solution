// Codeforces 189A - Cut Ribbon.cpp
...
int ar[10], dp[4][SIZE], n;

int Function(int pos, int taken, int cnt)
{
    if(pos >= 3)
    {
        if(taken == n)
    ...
    int ret1 = 0, ret2 = 0;
    if(taken + ar[pos] <= n)
        ret1 = Function(pos, taken + ar[pos], cnt + 1);

    ret2 = Function(pos + 1, taken, cnt);
