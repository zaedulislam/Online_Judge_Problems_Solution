// Codeforces 387A - George and Sleep.cpp
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, m, k, cnt = 0, len, currTimeH, currTimeM, sTimeH, sTimeM, ansH, ansM;

    cin >> S1 >> S2;

    currTimeH = S1[0] - '0';
    currTimeH = currTimeH * 10 + S1[1] - '0';

    currTimeM = S1[3] - '0';
    currTimeM = currTimeM * 10 + S1[4] - '0';


    sTimeH = S2[0] - '0';
    sTimeH = sTimeH * 10 + S2[1] - '0';

    sTimeM = S2[3] - '0';
    sTimeM = sTimeM * 10 + S2[4] - '0';


    if(sTimeM > currTimeM)
    {
        ansM = 60 - (sTimeM - currTimeM);
        sTimeH++;
    }
    else
        ansM = currTimeM - sTimeM;


    if(sTimeH > currTimeH)
        ansH = 24 - (sTimeH - currTimeH);
    else
        ansH = currTimeH - sTimeH;
