// Codeforces 988C - Equal Sums.cpp
...
int main()
{
	...
	///READ();
	///WRITE();
	int T, I, J, K, N, n, m, k, cnt = 0, len, idx = 0;
    ...
    sort(tup, tup + idx, Compare);


    for(I = 0; I < idx - 1; I++)
    {
        if((tup[I].sum == tup[I + 1].sum) && (tup[I].seq != tup[I + 1].seq))
        {
            cout << "YES\n";
            cout << tup[I].seq + 1 << ' ' << tup[I].pos + 1 << NL << tup[I + 1].seq + 1 << ' ' << tup[I + 1].pos + 1 << NL;
            return 0;
        }
    }
