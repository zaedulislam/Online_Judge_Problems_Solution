...
int main()
{
	...
	int tcases, I, J, K, N, n, m, cnt = 0, len, t, left = 0, sum = 0, ans = 0;
    //READ();
    //WRITE()
    cin >> n >> t;
	...
	for(I = 0; I < n; I++)
	{
		sum += ar[I];	// add item to current Sum
		cnt++;
		...
		while(sum > t)	// if current Sum is greater that given time
		{
			sum -= ar[left];	// remove the leftmost element
			left++;	// increment left pointer
			cnt--;
		}
		ans = MAX(ans, cnt);	// at each iteration update ans with maximum book window length till now
	}
	cout << ans << NL;
