// Codeforces 550B - Preparing Olympiad [Using Backtracking].cpp
...
void F(int pos, int cnt, int sum, int mx, int mn)
{
	...
	F(pos + 1, cnt, sum, mx, mn);

	mx = max(ar[pos], mx);
	mn = min(ar[pos], mn);

	F(pos + 1, cnt + 1, sum + ar[pos], mx, mn);
